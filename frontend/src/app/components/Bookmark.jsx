...
export default function Bookmark() {
  ...
  useEffect(() => {
    ...
    if (!token || role !== "buyer") {
      alert("Access denied! Only buyers can view favorites.");
      router.push("/");
      return;
    }
  ...
  const handleRating = async (type) => {
    ...
    const updatedSelected = Object.values(products)
      .flat()
  ...
  const handleBookmarkToggle = async (product_id, isBookmarked) => {
    const token = localStorage.getItem("token");
    const url = `http://localhost:5000/bookmark-product`;
    await fetch(url, {
          ...
          <div className="relative h-56 overflow-hidden bg-slate-900">
            <div
              ...
              style={{
                ...
                transform: `translateY(${(scrollY * 0.05 * (idx % 4)) % 10}px)`,
              }}
            >
              <img
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                handleBookmarkToggle(product.product_id, true);
              }}
              className="absolute top-3 right-3 w-9 h-9 bg-rose-500/90 backdrop-blur-sm hover:bg-rose-600 text-white rounded-full transition-all hover:scale-110 flex items-center justify-center font-bold border border-rose-400/50 shadow-lg"
            >
              ✕
            </button>
          </div>
    ...
    return (
      <div className="min-h-screen flex justify-center items-center" style={{background: "linear-gradient(180deg, #0a0e1a 0%, #1a1f2e 100%)"}}>
        <div className="text-center">
          ...
          <p className="text-white text-xl font-semibold">Loading Favorites...</p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen relative overflow-x-hidden" style={{background: "linear-gradient(180deg, #0a0e1a 0%, #1a1f2e 100%)"}}>
      ...
      <div className="relative pt-8 pb-6 px-4 md:px-8">
        <div className="max-w-7xl mx-auto text-center">
          <h1 className="text-5xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-rose-400 via-pink-500 to-purple-600 mb-3 tracking-tight">
            My Favorites
          </h1>
          <p className="text-slate-400 text-lg">Your bookmarked products collection</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 pb-16">
          ...
          {["study_tools", "clothes", "food"].map((tab) => (
            <button
              ...
              className={`px-6 py-3 rounded-full font-semibold transition-all duration-300 ${
                activeTab === tab
                  ? "bg-gradient-to-r from-rose-400 to-pink-500 text-slate-900 shadow-lg shadow-rose-500/50 scale-105"
        ...
        {activeTab === "study_tools" && renderGrid(products.study_tools)}
        {activeTab === "clothes" && renderGrid(products.clothes)}
        {activeTab === "food" && renderGrid(products.food)}
      </div>
          ...
          <div className="bg-slate-900 border border-slate-700/50 w-full max-w-5xl rounded-3xl shadow-2xl flex flex-col md:flex-row relative overflow-hidden animate-fadeIn">
            ...
            <div className="w-full md:w-1/2 p-8 flex flex-col justify-between overflow-y-auto" style={{maxHeight: "600px"}}>
              <div>
                ...
                <p className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-rose-400 to-pink-500 mb-6">
      ...
      {showPurchaseModal && selectedProduct && (
        <PurchaseModal
          ...
          onSuccess={() => {
            ...
            setSelectedProduct(null); // ✅ closes the product preview too
          }}
        />
      )}
      ...
      {showFeedbackModal && selectedProduct && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex justify-center items-center z-[60] p-4">
          <div className="bg-slate-900 border border-slate-700/50 w-full max-w-3xl rounded-3xl shadow-2xl p-8 relative">
            ...
            <h3 className="text-3xl font-black mb-6 text-transparent bg-clip-text bg-gradient-to-r from-rose-400 to-pink-500">
            ...
            <div className="max-h-72 overflow-y-auto bg-slate-800/40 backdrop-blur-sm p-5 rounded-2xl border border-slate-700/50 custom-scrollbar">
              {loadingFeedback ? (
                <div className="text-center py-8">
                  <div className="inline-block animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-rose-400 mb-3"></div>
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <p className="text-white font-bold text-lg flex items-center gap-2">
                        <span className="w-8 h-8 rounded-full bg-gradient-to-r from-rose-400 to-pink-500 grid place-items-center text-slate-900 text-sm font-black">
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button
                ...
                className="bg-gradient-to-r from-rose-500 to-pink-600 hover:brightness-110 text-white px-6 py-3 rounded-xl font-bold transition-all hover:scale-105 shadow-lg shadow-rose-500/30"
              >
                ✍️ Add Feedback
              </button>
              <button
      ...
      {showAddFeedbackModal && selectedProduct && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-md flex justify-center items-center z-[70] p-4">
          <div className="bg-slate-900 border border-slate-700/50 w-full max-w-lg rounded-3xl shadow-2xl p-8 relative">
            ...
            <h4 className="text-2xl font-black mb-5 text-transparent bg-clip-text bg-gradient-to-r from-rose-400 to-pink-500">
              Add Your Feedback
            </h4>
            ...
            <textarea
              ...
              rows={6}
              className="w-full p-4 rounded-2xl bg-slate-800/60 backdrop-blur-sm border border-slate-700/50 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-rose-400 focus:border-transparent transition resize-none"
            />
